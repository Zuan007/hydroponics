...
#define ECHOPIN 4 //
#define TRIGPIN 5 //
#define RELAY1 36 //snap a
#define RELAY2 34 //snap b
#define RELAY3 32 //ph +
#define RELAY4 30 //ph -
#define RELAY5 28 //water pump
#define RELAY6 38
...
void loop() {
  ...
  //control
  controlMoisture(moistVal);
  controlTDS(tdsVal);
  controlPHUp(phVal);
  controlPHDown(phVal);
  controlEC(ecVal);
  controlHumidity(humVal);
  controlLux(luxVal);
}
...
void controlMoisture(float moistVal) {
  if (moistVal <= 300) {
...
void controlPHDown(float phVal) {
  if (phVal >= 6.8) {
...
void controlHumidity(float humVal) {
  if (humVal <= 50) {
    digitalWrite(RELAY5, HIGH);
  } else {
    digitalWrite(RELAY5, LOW);
